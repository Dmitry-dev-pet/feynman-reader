...
\delta=\frac{x}{L}\,\frac{d}{\lambdabar}. (15.28)
...
\text{Magnetic change in phase}=\frac{q}{\hbar}\kern{-2ex} \underset{\text{trajectory}}{\int}\kern{-2ex}\mathbf{A}\cdot d\mathbf{s}. (15.29)
...
\Phi_1=\Phi_1(B=0)+\frac{q}{\hbar} \int_{(1)}\mathbf{A}\cdot d\mathbf{s}. (15.30)

Similarly, the phase for trajectory (2) is

\Phi_2=\Phi_2(B=0)+\frac{q}{\hbar} \int_{(2)}\mathbf{A}\cdot d\mathbf{s}. (15.31)

The interference of the waves at the detector depends on the phase difference

\begin{gathered} \delta=\Phi_1(B=0)-\Phi_2(B=0)+\\[1ex] \frac{q}{\hbar}\int_{(1)}\mathbf{A}\cdot d\mathbf{s}- \frac{q}{\hbar}\int_{(2)}\mathbf{A}\cdot d\mathbf{s}. \end{gathered} (15.32)
...
\delta=\delta(B=0)+\frac{q}{\hbar} \oint_{(1–2)}\mathbf{A}\cdot d\mathbf{s}. (15.33)
...
\delta=\delta(B=0)+\frac{q}{\hbar}\, \begin{bmatrix} \text{flux of $\mathbf{B}$}\\[-.5ex] \text{between $(1)$ and $(2)$} \end{bmatrix}, (15.34)
...
### Figure Ch15-F6
Caption: Figure 15-6.The magnetic field and vector potential of a long solenoid.
Image: figures/Ch15-F6.svg
![Figure 15-6.The magnetic field and vector potential of a long solenoid.](figures/Ch15-F6.svg)
...
### Figure Ch15-F7
Caption: Figure 15-7.A magnetic field can influence the motion of electrons even though it exists only in regions where there is an arbitrarily small probability of finding the electrons.
Image: figures/Ch15-F7.svg
![Figure 15-7.A magnetic field can influence the motion of electrons even though it exists only in regions where there is an arbitrarily small probability of finding the electrons.](figures/Ch15-F7.svg)

We compare the situation with and without a current through the solenoid. If we have no current, we have no \mathbf{B} or \mathbf{A} and we get the original pattern of electron intensity at the backstop. If we turn the current on in the solenoid and build up a magnetic field \mathbf{B} inside, then there is an \mathbf{A} outside. There is a shift in the phase difference proportional to the circulation of \mathbf{A} outside the solenoid, which will mean that the pattern of maxima and minima is shifted to a new position. In fact, since the flux of \mathbf{B} inside is a constant for any pair of paths, so also is the circulation of \mathbf{A} . For every arrival point there is the same phase change; this corresponds to shifting the entire pattern in x by a constant amount, say x_0 , that we can easily calculate. The maximum intensity will occur where the phase difference between the two waves is zero. Using Eq. ( 15.33) or Eq. ( 15.34) for \delta and Eq. ( 15.28) for x , we have

x_0=-\frac{L}{d}\,\lambdabar\,\frac{q}{\hbar} \oint_{(1–2)}\mathbf{A}\cdot d\mathbf{s}, (15.35)

or

x_0=-\frac{L}{d}\,\lambdabar\,\frac{q}{\hbar}\, \begin{bmatrix} \text{flux of $\mathbf{B}$}\\[-.5ex] \text{between $(1)$ and $(2)$} \end{bmatrix}. (15.36)

The pattern with the solenoid in place should appear 1 as shown in Fig. 15–7. At least, that is the prediction of quantum mechanics.
...
### Figure Ch15-F8
Caption: Figure 15-8.The shift of the interference pattern due to a strip of magnetic field.
Image: figures/Ch15-F8.svg
![Figure 15-8.The shift of the interference pattern due to a strip of magnetic field.](figures/Ch15-F8.svg)

Now we wish to continue in our analysis a little further. We will show the connection between the quantum-mechanical formula and the classical formula—to show why it turns out that if we look at things on a large enough scale it will look as though the particles are acted on by a force equal to q\mathbf{v}\times{} the curl of \mathbf{A} . To get classical mechanics from quantum mechanics, we need to consider cases in which all the wavelengths are very small compared with distances over which external conditions, like fields, vary appreciably. We shall not prove the result in great generality, but only in a very simple example, to show how it works. Again we consider the same slit experiment. But instead of putting all the magnetic field in a very tiny region between the slits, we imagine a magnetic field that extends over a larger region behind the slits, as shown in Fig. 15–8. We will take the idealized case where we have a magnetic field which is uniform in a narrow strip of width w , considered small as compared with L . (That can easily be arranged; the backstop can be put as far out as we want.) In order to calculate the shift in phase, we must take the two integrals of \mathbf{A} along the two trajectories (1) and (2) . They differ, as we have seen, merely by the flux of \mathbf{B} between the paths. To our approximation, the flux is Bwd . The phase difference for the two paths is then

\delta=\delta(B=0)+\frac{q}{\hbar}\,Bwd. (15.37)

We note that, to our approximation, the phase shift is independent of the angle. So again the effect will be to shift the whole pattern upward by an amount \Delta x . Using Eq. ( 15.35),

\Delta x=-\frac{L\lambdabar}{d}\,\Delta\delta= -\frac{L\lambdabar}{d}\,[\delta-\delta(B=0)].

Using ( 15.37) for \delta-\delta(B=0) ,

\Delta x=-L\lambdabar\,\frac{q}{\hbar}\,Bw. (15.38)

Such a shift is equivalent to deflecting all the trajectories by the small angle \alpha (see Fig. 15–8), where

\alpha=\frac{\Delta x}{L}=-\frac{\lambdabar}{\hbar}\,qBw. (15.39)

### Figure Ch15-F9
Caption: Figure 15-9.Deflection of a particle due to passage through a strip of magnetic field.
Image: figures/Ch15-F9.svg
![Figure 15-9.Deflection of a particle due to passage through a strip of magnetic field.](figures/Ch15-F9.svg)

Now classically we would also expect a thin strip of magnetic field to deflect all trajectories through some small angle, say \alpha' , as shown in Fig. 15–9 (a). As the electrons go through the magnetic field, they feel a transverse force q\mathbf{v}\times\mathbf{B} which lasts for a time w/v . The change in their transverse momentum is just equal to this impulse, so

\Delta p_x=-qwB. (15.40)

The angular deflection [Fig. 15–9 (b)] is equal to the ratio of this transverse momentum to the total momentum p . We get that

\alpha'=\frac{\Delta p_x}{p}=-\frac{qwB}{p}. (15.41)
...
From the analysis we see how it is that the vector potential which appears in quantum mechanics in an explicit form produces a classical force which depends only on its derivatives. In quantum mechanics what matters is the interference between nearby paths; it always turns out that the effects depend only on how much the field \mathbf{A} changes from point to point, and therefore only on the derivatives of \mathbf{A} and not on the value itself. Nevertheless, the vector potential \mathbf{A} (together with the scalar potential \phi that goes with it) appears to give the most direct description of the physics. This becomes more and more apparent the more deeply we go into the quantum theory. In the general theory of quantum electrodynamics, one takes the vector and scalar potentials as the fundamental quantities in a set of equations that replace the Maxwell equations: \mathbf{E} and \mathbf{B} are slowly disappearing from the modern expression of physical laws; they are being replaced by \mathbf{A} and \phi .
